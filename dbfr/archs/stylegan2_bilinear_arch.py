...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, x, style,input_is_style=False):
        """Forward function.
        ...
        # self.weight: (1, c_out, c_in, k, k); style: (b, 1, c, 1, 1)
            ...
            style = style.view(b, 1, c, 1, 1)
        # print(self.weight.shape,style.shape)
        ...
        r_style = style.squeeze()
        if len(r_style.shape) == 1:
            r_style = r_style.unsqueeze(0) 
        return out,r_style
...
class StyleConv(nn.Module):
    ...
    def forward(self, x, style, noise=None,input_is_style=False):
        # modulate
        out,return_style  = self.modulated_conv(x, style,input_is_style)
        # noise injection
        if noise is None:
            b, _, h, w = out.shape
...
class ToRGB(nn.Module):
    ...
    def forward(self, x, style, skip=None):
        ...
        Returns:
            Tensor: RGB images.
        """
        out,_ = self.modulated_conv(x, style)
        out = out + self.bias
        if skip is not None:
            if self.upsample:
                skip = F.interpolate(
